...
- 单一 checkpoint、单一语言、单一设备，固定温度下的贪心解码。这里没有任何结论适用于英文或 typed-decisions checkpoint。
...
由 GaotianJin 贡献，harness、审计与测试借助 AI 协助完成。用例文本为贡献者本人所写。不包含任何用户数据、录音、凭据或模型权重；checkpoint 仍遵循其自身许可。本目录与仓库其余部分一样采用 MIT 许可。
